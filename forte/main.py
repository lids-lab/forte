...
def apply_psp(batch, p=0.10):
    """Partial Schema Perturbation (handoff Section 10): per semantic type, replace
    ~p of col_name_values with another same-type column-name embedding from the batch."""
    cnv = batch["col_name_values"]
    B, S, D = cnv.shape
    sem = batch["sem_types"].reshape(-1)
    pad = batch["is_padding"].reshape(-1)
    flat = cnv.reshape(B * S, D).clone()
    for i in range(4):
        idx = ((sem == i) & ~pad).nonzero(as_tuple=True)[0]
        n = idx.numel()
        if n < 2:
            continue
        k = max(1, int(p * n))
        chosen = idx[torch.randperm(n, device=idx.device)[:k]]
        donors = idx[torch.randint(0, n, (k,), device=idx.device)]
        flat[chosen] = flat[donors]
    out = dict(batch)
    out["col_name_values"] = flat.reshape(B, S, D)
    return out


def perturb_role_C(C, p=0.10):
    """PSP-perturbs-C: swap ~p of the frozen role table's rows among themselves
    (row 0 = no-edge is left intact)."""
    R = C.shape[0]
    if R <= 2:
        return C
    Cp = C.clone()
    k = max(1, int(p * (R - 1)))
    idx = 1 + torch.randperm(R - 1, device=C.device)[:k]
    Cp[idx] = C[idx[torch.randperm(k, device=C.device)]]
    return Cp


def psp_curriculum_weight(steps, max_steps, alpha, schedule):
    """w(t): 0 until 20% of training, then ramp to alpha by 50% (linear) or jump (step)."""
    if steps < 0.2 * max_steps:
        return 0.0
    if schedule == "step":
        return alpha
    if steps < 0.5 * max_steps:
        return alpha * (steps - 0.2 * max_steps) / (0.3 * max_steps)
    return alpha
...
def main(
    ...
    # FORTE: supply the frozen global FK-role table C (kept float32, non-persistent)
    net.set_fk_role_C(dataset.fk_role_C)
    opt = optim.AdamW(
        net.parameters(),
        lr=lr,
        weight_decay=wd,
    ...
    if ddp:
        # find_unused_parameters=True tolerates FORTE's conditional graph
        # (CLP FK-masking + the PSP second forward) under multi-GPU DDP.
        ...
        with torch.inference_mode():
            for (
                ...
                while True:
                    ...
                    loss, _loss_clp, yhat_dict = net(batch)
    ...
    while steps < max_steps:
        ...
        while steps < max_steps:
            ...
            psp_w = (
                psp_curriculum_weight(steps, max_steps, psp_max_weight, psp_schedule)
                if use_psp
                else 0.0
            )
            do_psp = psp_w > 0 and random.random() < psp_input_prob

            opt.zero_grad(set_to_none=True)
            loss_mcp, loss_clp, _yhat_dict = net(batch)
            loss = loss_mcp + clp_weight * loss_clp

            if do_psp:
                batch_psp = apply_psp(batch)
                C_ov = perturb_role_C(_unwrap(net).fk_role_C) if psp_perturb_roles else None
                loss_psp, _, _ = net(batch_psp, skip_clp=True, C_override=C_ov)
                loss = loss + psp_w * loss_psp
