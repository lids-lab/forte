    ...
    fn into_pyobject<'a>(self, py: Python<'a>) -> PyResult<Vec<PyObject>> {
        Ok(vec![
            ...
            ("is_padding", PyArray1::from_vec(py, self.is_padding))
                .into_py_any(py)
                .unwrap(),
            ("dataset_idxs", PyArray1::from_vec(py, self.dataset_idxs))
                .into_py_any(py)
                .unwrap(),
...
impl Sampler {
    ...
    fn batch(&self, batch_idx: usize) -> Vecs {
        ...
        // tag every cell of row i with its source dataset index (all cells in a
        // row share one DB; needed Python-side to pick the right col_kind / fk_roles)
        for i in 0..self.batch_size {
            let j = batch_idx * self.batch_size * self.world_size + self.rank * self.batch_size + i;
            let j = j % self.items.len();
            let ds = self.items[j].dataset_idx;
            for s in 0..self.seq_len {
                vecs.dataset_idxs[i * self.seq_len + s] = ds;
            }
        }
        vecs
    }
